...
public class SmoothLook : MonoBehaviour
{
    ...
    void Follow() 
    {
        if (target != null) 
        {
            ...
            if (isLocal) pos = target.position + (target.up * offset.y) + (target.right * offset.x) + (target.forward * offset.z);
            else pos = target.position + offset;
        }

        transform.LookAt(Vector3.SmoothDamp(transform.position + (transform.forward * Vector3.Distance(transform.position, pos)), pos, ref velocity, smoothTime));
    }
}
